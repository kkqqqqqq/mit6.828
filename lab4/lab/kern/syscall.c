...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((uintptr_t)srcva < UTOP) {
		...
		if ((uintptr_t)srcva % PGSIZE)
			return -E_INVAL;
		//-E_INVAL if srcva < UTOP and perm is inappropriate (see sys_page_alloc).
		//PTE_U | PTE_P must be set, 
		if ((perm & (PTE_U | PTE_P)) == 0)
			return -E_INVAL;            
		//-E_INVAL if srcva < UTOP but srcva is not mapped in the caller's
		...
		//	-E_INVAL if (perm & PTE_W), but srcva is read-only in the
       		//  current environment's address space.
		if((perm & PTE_W) && (*pte & 0x800)) return -E_INVAL;
		//	-E_INVAL if (perm & PTE_W), but srcva is read-only in the
        	//	current environment's address space.
		//	-E_NO_MEM if there's not enough memory to map srcva in envid's
        	//	address space.
		if (page_insert(e->env_pgdir, srcpp, e->env_ipc_dstva, perm))
			return -E_NO_MEM;
...
//sys_ipc_try_send中要注意sys_ipc_recv中在调用sched_yield()让出cpu后就不会返回了,那么用户程序要获得sys_ipc_recv的返回值只能通过在sys_ipc_try_send中设置eax寄存器:
	e->env_tf.tf_regs.reg_eax = 0;
	return 0;
